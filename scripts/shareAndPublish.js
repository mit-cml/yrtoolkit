...
$(document).ready( function() {
  ...
  $("button.shareApp").click(function(e) {
    ...
    if (projectUrl.indexOf("ai2.appinventor.mit.edu/?galleryId=") === 0) {
      ...
      bd.util.ajaxPost(url + "submitProject/" + serverTutorialId + "/" + appGalleryId + "/","appName=" + appName + "&username=" + appUsername ,function(response){
  ...
  bd.util.ajaxGet(url + "getTutorialProjects/" + serverTutorialId + "/",function(response){
    //console.log(response)
    var galleryAppIds = JSON.parse(response);
